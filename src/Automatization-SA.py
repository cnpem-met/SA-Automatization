...
def generate_localFrames(lookuptable, pts_ML, mode='full', convers_type='direct'):
...
def plot_girder_deviation(results_df, analysis_type, plots_args_dict, freezePlot=False):
    ...
    while (i < dataset.iloc[:, 0].size):
        ...
        if (current_girder[4:] == 'B03' or current_girder[4:] == 'B11'):
            if (point_name[-2:] != "B1" and point_name[-2:] != "MR"):
                pts_girder.append(
                    [dataset.iloc[i, 0], dataset.iloc[i, 1], dataset.iloc[i, 2]])
        ...
        while (current_girder == dataset.index[j][:7]):
            ...
            if (current_girder[4:] == 'B03' or current_girder[4:] == 'B11'):
                if (point_name[-2:] != "B1" and point_name[-2:] != "MR"):
                    pts_girder.append(
                        [dataset.iloc[j, 0], dataset.iloc[j, 1], dataset.iloc[j, 2]])
                    pts_name.append(point_name)
            else:
                pts_girder.append(
                    [dataset.iloc[j, 0], dataset.iloc[j, 1], dataset.iloc[j, 2]])
        ...
        if (current_girder[4:] != 'B05' and current_girder[4:] != 'B09'):
            ...
            if(error):
                ...
                B2_new = translate(B2_new, -B2[0], -B2[1], -B2[2], 'direct')
                ...
                B2MR_new = rotate(B2MR_new, 0, 0, 20, 'direct')
...
""" Regras para o cálculo:
    SR:
        B01->B02: reta entre centroides
        B02->B04: arco com raio=raio_centroide_B03
        B04->B06: arco com raio=raio_centroid_B05
        B06->B07: reta entre centroides
        B07->B08: reta entre centroides
        B08->B10: arco com raio=raio_centroide_B09
        B10->B01: arco com raio=raio_centroide_B11 """


def calculate_machines_length_method2(centr_ML_cylind, coord='cylind'):
    length = 0
    i = 0
    while (i < centr_ML_cylind.iloc[:, 0].size):
        g1 = centr_ML_cylind.index[i]
        g1_type = g1[4:]
        g1_theta, g1_R, g1_Z = centr_ML_cylind.loc[g1,
                                                   'Theta'], centr_ML_cylind.loc[g1, 'r'], centr_ML_cylind.loc[g1, 'z']

        if i == 0:
            S01B01 = [g1_theta, g1_R, g1_Z]

        if (g1_type == "B01" or g1_type == "B06" or g1_type == "B07"):
            g2 = centr_ML_cylind.index[i+1]
            g2_theta, g2_R, g2_z = centr_ML_cylind.loc[g2,
                                                       'Theta'], centr_ML_cylind.loc[g2, 'r'], centr_ML_cylind.loc[g2, 'z']

            # ** aqui não to considerando os desníveis entre berços no cálculo!
            dist = math.sqrt(g1_R**2 + g2_R**2 - 2 * g1_R *
                             g2_R * math.cos(math.radians(g2_theta - g1_theta)))
            length += dist
            ...
            if (g1 != "S20-B10"):
                g2 = centr_ML_cylind.index[i+2]
                g2_theta, g2_R, g2_Z = centr_ML_cylind.loc[g2,
                                                           'Theta'], centr_ML_cylind.loc[g2, 'r'], centr_ML_cylind.loc[g2, 'z']
            else:
                g2_theta = S01B01[0]
                g1_theta = g1_theta - 360  # adequação para transição do 359° -> 0°

            g_mid = centr_ML_cylind.index[i+1]
            R_ref = centr_ML_cylind.loc[g_mid, 'r']

            #dist = (g2_theta - g1_theta) * R_ref
            dist = ((g2_theta - g1_theta) * math.pi * R_ref) / 180
            length += dist
...
def calculate_machines_length(centr_ML_cylind, coord='cylind'):
    length = 0
    i = 0

    if (coord == 'cylind'):
        R_mean = np.mean(np.array(centr_ML_cylind.loc[:, 'r']))
    ...
    while (i < centr_ML_cylind.iloc[:, 0].size):
        ...
        if (coord == 'cylind'):
            ...
            g1_x, g1_y, g1_z = centr_ML_cylind.loc[g1,
                                                   'x'], centr_ML_cylind.loc[g1, 'y'], centr_ML_cylind.loc[g1, 'z']

        if i == 0:
            if (coord == 'cylind'):
                S01B01 = [g1_theta, g1_R, g1_z]
            else:
                S01B01 = [g1_x, g1_y, g1_z]

        if (coord == 'cylind'):
            # assumindo que a lista de thetas está ordenada de forma descrescente...
            if (g1 != "S20-B11"):
                g2 = centr_ML_cylind.index[i+1]
                g2_theta, g2_R, g2_z = centr_ML_cylind.loc[g2,
                                                           'Theta'], centr_ML_cylind.loc[g2, 'r'], centr_ML_cylind.loc[g2, 'z']
            else:
                g2_theta, g2_z = S01B01[0], S01B01[2]

            if (g2_theta > g1_theta):
                g1_theta += 360

            dist = (math.radians(g1_theta) - math.radians(g2_theta)) * R_mean

            # if (g1 != "S20-B11"):
            #     g2 = centr_ML_cylind.index[i+1]
            #     g2_theta, g2_R, g2_z = centr_ML_cylind.loc[g2,'Theta'], centr_ML_cylind.loc[g2,'r'], centr_ML_cylind.loc[g2,'z']

            # else:
            #     g2_theta, g2_z = S01B01[0], S01B01[2]
            #     #g1_theta = g1_theta - 360 # adequação para transição do 359° -> 0°

            #dist = (math.radians(g2_theta) - math.radians(g1_theta)) * R_mean
            # # variação do método com acréscimo dos desníveis:
            # d_y = (math.radians(g2_theta) - math.radians(g1_theta)) * R_mean
            # d_z = g2_z - g1_z
            # dist = math.sqrt (d_y**2 + d_z**2)

        else:
            if (g1 != "S20-B11"):
                g2 = centr_ML_cylind.index[i+1]
                g2_x, g2_y, g2_z = centr_ML_cylind.loc[g2,
                                                       'x'], centr_ML_cylind.loc[g2, 'y'], centr_ML_cylind.loc[g2, 'z']
            else:
                g2_x, g2_y, g2_z = S01B01[0], S01B01[1], S01B01[2]

            dist = math.sqrt((g2_x - g1_x)**2 + (g2_y - g1_y)
                             ** 2 + (g2_z - g1_z)**2)
...
if __name__ == "__main__":
    ...
    frameTransf_lookup = load_and_sort_dataframe(
        "../data/input/frames_table_fullpre.xlsx", "Planilha4", has_header=True)
    ptsML_nominals = load_and_sort_dataframe(
        "../data/input/SR_nominals.xlsx", "Planilha2", has_header=False)
    ptsML_measured = load_and_sort_dataframe(
    ...
    ptsLocal_nominals = generate_localFrames(
        frameTransf_lookup, ptsML_nominals)
    ptsLocal_measured = generate_localFrames(
        frameTransf_lookup, ptsML_measured)
    ...
    plot_girder_deviation(inout_delta, 'inout_delta',
                          plot_args, freezePlot=True)

    # %%

    centr_meas_ML_cart1 = calculate_centroids(ptsML_measured, 'measured')
    centr_meas_ML_cyl1 = cartesian_to_cylind_coord(centr_meas_ML_cart1)

    centr_nom_ML_cart1 = calculate_centroids(ptsML_nominals, 'nominal')
    centr_nom_ML_cyl1 = cartesian_to_cylind_coord(centr_nom_ML_cart1)

    print("Resultados com centroides calculados a partir de pontos em ML (coord. CILÍNDRICAS)")
    SR_length_meas2 = calculate_machines_length(centr_meas_ML_cyl1)
    SR_length_nom2 = calculate_machines_length(centr_nom_ML_cyl1)
    print("\tmedido: "+str(SR_length_meas2))
    print("\tnominal: "+str(SR_length_nom2))
    print("\tdelta: " + str(SR_length_nom2 - SR_length_meas2))

    print("------------------------------")

    print("Resultados com centroides calculados a partir de pontos em ML (coord. CARTESIANAS)")
    SR_length_meas2 = calculate_machines_length(
        centr_meas_ML_cart1, coord='cart')
    SR_length_nom2 = calculate_machines_length(
        centr_nom_ML_cart1, coord='cart')
    print("\tmedido: "+str(SR_length_meas2))
    print("\tnominal: "+str(SR_length_nom2))
    print("\tdelta: " + str(SR_length_nom2 - SR_length_meas2))
    ...
    centr_meas_ML_cart2 = generate_localFrames(
        frameTransf_lookup, centroids_measured, convers_type='indirect')
    centr_meas_ML_cyl2 = cartesian_to_cylind_coord(centr_meas_ML_cart2)

    centr_nom_ML_cart2 = generate_localFrames(
        frameTransf_lookup, centroids_nominal, convers_type='indirect')
    centr_nom_ML_cyl2 = cartesian_to_cylind_coord(centr_nom_ML_cart2)

    print("Resultados com centroides calculados nos frames locais (coord. CILÍNDR.)")
    SR_length_meas2 = calculate_machines_length(centr_meas_ML_cyl2)
    SR_length_nom2 = calculate_machines_length(centr_nom_ML_cyl2)
    print("\tmedido: "+str(SR_length_meas2))
    print("\tnominal: "+str(SR_length_nom2))
    print("\tdelta: " + str(SR_length_nom2 - SR_length_meas2))

    print("------------------------------")
    print("Resultados com centroides calculados nos frames locais (coord. CART.)")
    SR_length_meas2 = calculate_machines_length(
        centr_meas_ML_cart2, coord='cart')
    SR_length_nom2 = calculate_machines_length(
        centr_nom_ML_cart2, coord='cart')
    print("\tmedido: "+str(SR_length_meas2))
    print("\tnominal: "+str(SR_length_nom2))
    print("\tdelta: " + str(SR_length_nom2 - SR_length_meas2))
...
Próximas implementações:
    ...
    - [ANDAMENTO] cálculo do comprimento da máquina e comparação de resultados
    - implementar interface gráfica
    - criação de nominais "suavizados" ****
    - ferramenta pra analisar o frame (translação e rotação) igual ao SA
